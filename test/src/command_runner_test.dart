...
import 'package:fpx/src/command_runner.dart';
...
void main() {
  group('FpxCommandRunner', () {
    late PubUpdater pubUpdater;
    late Logger logger;
    late FpxCommandRunner commandRunner;
    ...
    setUp(() {
      ...
      commandRunner = FpxCommandRunner(
        logger: logger,
    ...
    test('can be instantiated without an explicit analytics/logger instance',
        () {
      final commandRunner = FpxCommandRunner();
    ...
    group('--verbose', () {
      ...
      test('enables verbose logging for sub commands', () async {
        ...
        verify(() => logger.detail('  Top level options:')).called(1);
        verify(() => logger.detail('  - verbose: true')).called(1);
        verify(() => logger.detail('  Command: add')).called(1);
        verify(() => logger.detail('    Command options:')).called(1);
        verify(() => logger.detail('    - name: test_widget')).called(1);
      });
    });
  });
}
